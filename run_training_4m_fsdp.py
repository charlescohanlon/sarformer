...
import src.utils.fsdp_utils as fsdp_utils
...
from src.models.fm_utils import EncoderBlock, DecoderBlock
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        "4M pre-training script (using FSDP)", add_help=False
    )
    ...
    parser.add_argument(
        "--no_use_act_checkpoint", action="store_false", dest="use_act_checkpoint"
    )
    ...
    parser.add_argument(
        "--num_input_tokens", type=int, default=128, help="Token budget for the input"
    )
    parser.add_argument(
        "--num_target_tokens", type=int, default=128, help="Token budget for the target"
    )
    parser.add_argument(
        "--min_input_tokens",
        type=int,
...
def setup_data(args):
    ...
    args.in_domains = sorted(
        set.union(*[set(cfg["in_domains"].split("-")) for cfg in train_config.values()])
    )
    args.out_domains = sorted(
        set.union(
            *[set(cfg["out_domains"].split("-")) for cfg in train_config.values()]
        )
    )
    ...
    if any([cfg["data_path"].startswith("s3") for cfg in train_config.values()]):
        utils.s3_utils.override_wds_s3_tar_loading(
            args.s3_data_endpoint,
            args.s3_multipart_threshold_mb,
            args.s3_multipart_chunksize_mb,
            args.s3_max_io_queue,
        )
    ...
    for dataset_name, dataset_cfg in train_config.items():
        ...
        dataset_mod_info, sampling_weights = setup_sampling_mod_info(
            dataset_cfg, modality_info
        )
    ...
    if "val" in data_config:
        ...
        for dataset_name, dataset_cfg in val_config.items():
            ...
            dataset_mod_info, sampling_weights = setup_sampling_mod_info(
                train_config[dataset_name], modality_info
            )
        ...
        data_loaders_fixed_eval = (
            data_loaders_fixed_eval if data_loaders_fixed_eval else None
        )
...
def main(args):
    ...
    if args.epochs < 0:
        if args.total_tokens < 0:
            print(
                "Epochs and total tokens are both set to negative values, stopping training."
            )
            exit(1)
        else:
    ...
    if args.warmup_epochs < 0 and args.warmup_steps < 0:
        if args.warmup_tokens < 0:
            print(
                "Warmup epochs, steps and total tokens all set to negative values, stopping training."
            )
            exit(1)
        else:
    ...
    if args.cooldown_epochs < 0 and args.cooldown_steps < 0:
        if args.cooldown_tokens < 0 and args.lr_schedule in ["inverse_sqrt"]:
            print(
                "Cooldown epochs, steps and total tokens all set to negative values, stopping training."
            )
            exit(1)
        else:
    ...
    if args.frozen_model_epochs <= 0:
        ...
        if args.frozen_model_tokens > 0:
            print(
                "Frozen_model_epochs and frozen_model_tokens are both non-negative, stopping training."
            )
            exit(1)
    ...
    lr_schedule_values = np.concatenate(
        (frozen_lr_schedule_values, main_lr_schedule_values)
    )
        ...
        if data_loaders_val is not None:
            for dataset_name, data_loader_val in data_loaders_val.items():
                ...
                print(
                    "Eval Stats:"
                    if not dataset_name
                    else f"Eval Stats ({dataset_name}):"
                )
                print(eval_stats)
                print()
        ...
        if data_loaders_fixed_eval is not None:
            for dataset_name, data_loader_fixed_eval in data_loaders_fixed_eval.items():
                prefix = (
                    "[Fixed Eval] "
                    if not dataset_name
                    else f"[Fixed Eval ({dataset_name})] "
                )
                ...
                print(
                    "Fixed Eval Stats:"
                    if not dataset_name
                    else f"Fixed Eval Stats ({dataset_name}):"
                )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                fsdp_utils.save_model_fsdp(
                    args=args, model=model, optimizer=optimizer, epoch=epoch
                )
        ...
        if data_loaders_val is not None and (
            (epoch + 1) % args.eval_freq == 0 or epoch + 1 == args.epochs
        ):
            for dataset_name, data_loader_val in data_loaders_val.items():
                prefix = "[Eval] " if not dataset_name else f"[Eval ({dataset_name})] "
                eval_stats = evaluate(
        ...
        if data_loaders_fixed_eval is not None and (
            (epoch + 1) % args.eval_freq == 0 or epoch + 1 == args.epochs
        ):
            for dataset_name, data_loader_fixed_eval in data_loaders_fixed_eval.items():
                prefix = (
...
def train_one_epoch(
    ...
    if frozen_model_epochs > 0 and epoch < frozen_model_epochs:
        if args.frozen_embedding_domain is None:
            ...
            model.module.freeze_params_except_specific_embeddings(
                args.frozen_embedding_domain
            )
    else:
    ...
    for step, x in enumerate(
        ...
        with nullcontext() if update_grad else model.no_sync():
            ...
            with torch.amp.autocast(
                ...
                mod_loss_values = {
                    f"{mod}_loss": l.item() for mod, l in mod_loss.items()
                }
        ...
        if log_writer is not None:
            ...
            if total_batch_size is not None:
                log_writer.update(
                    {
                        "input_tokens_seen_b": it
                        * (total_batch_size / accum_iter)
                        * num_input_tokens
                        / 1e9,
                        "target_tokens_seen_b": it
                        * (total_batch_size / accum_iter)
                        * num_target_tokens
                        / 1e9,
...
def evaluate(
    ...
    for x in metric_logger.log_every(
        data_loader, print_freq, iter_len=iter_len, header=header
    ):
