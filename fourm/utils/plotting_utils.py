...
from fourm.utils.data_constants import (
    ...
    COCO_SEMSEG_NUM_CLASSES,
)
from fourm.utils import destandardize, get_sentinel_to_id_mapping, merge_span_masking
...
def decode_input_rgb(mod_dict, key="rgb"):
    """
    Decodes (destandardizes) an RGB image from a model dictionary.

    Args:
        mod_dict (dict): Model output dictionary.
        key (str): Key of the RGB modality to decode.
    """
    img = destandardize(mod_dict[key]["tensor"])
...
def decode_tok_clip(
    mod_dict, tokenizers, key="tok_clip", image_size=224, patch_size=16
):
    """
...
def decode_tok_dinov2(
    mod_dict, tokenizers, key="tok_dinov2", image_size=224, patch_size=14
):
    """
    ...
    for decoded in all_decoded:
        ...
        if decoded != "none":
            try:
                ...
                print(
                    "Error in decoding human poses. Packages required for plotting may not be installed. Trace:"
                )
                print(e)
        imgs.append(img)
    ...
    for decoded in all_decoded:
        ...
        for part in parts:
            ...
            if metadata_type in metadata_transform.image_dim_modalities:
                ...
                vmin, vmax, bins = metadata_transform.metadata_min_max_bins[
                    metadata_type
                ]
        ...
        metadata_dict = {
            k: metadata_dict[k]
            for k in metadata_transform.metadata_id_map
...
def decode_text(mod_dict, key, text_tokenizer):
    ...
    for i in range(B):
        ...
        merged_seq = merge_span_masking(
            input_seq, target_seq, sentinel_ids=sentinel_ids
        )
...
def decode_sam_instances(
    ...
    def group_by_identifier(input_list, identifier):
        ...
        return [
            list(group)
            for key, group in groupby(input_list, lambda x: x == identifier)
            if not key
        ]
    ...
    for decoded in all_decoded:
        ...
        for part in group_by_identifier(parts, identifier="point"):
            ...
            for positions in group_by_identifier(part, identifier="polygon"):
                ...
                if None in [min_w, max_w, min_h, max_h] or (
                    min_w >= max_w or min_h >= max_h
                ):
                    continue
        ...
        # Decoded tokens
        tokens_per_sample = (
            torch.LongTensor(tokens_per_sample).reshape(-1, 4, 4).to(device)
        )
        ...
        for mask, bbox in zip(instances, bboxes_per_sample):
            ...
            for rms, rbs in zip(representive_masks, representive_bboxes):
                rm, rb = rms[0], rbs[0]
                sim_score = (
                    2 * ((rm * mask).sum() + 0.01) / (rm.sum() + mask.sum() + 0.01)
                )
                box_iou = iou(rb, bbox)
        ...
        for i, (rms, rbs) in enumerate(zip(representive_masks, representive_bboxes)):
            ...
            mask = cv2.resize(
                mask, (max_w - min_w, max_h - min_h), interpolation=cv2.INTER_CUBIC
            )
            max_w, max_h = min(max_w, final_instances.shape[1]), min(
                max_h, final_instances.shape[0]
            )
    ...
    all_sam_instances = (
        all_sam_instances[0]
        if len(all_sam_instances) == 1
        else np.stack(all_sam_instances)
    )
    ...
    for key in mod_dict:
        ...
        if k == "rgb":
            ...
            decoded = decode_tok_clip(
                mod_dict, tokenizers, key=key, image_size=res, patch_size=patch_size
            )
        elif k == "tok_dinov2":
...
def visualize_human_poses(pose, poses_tokenizer, mod_dict):
    ...
    for inst in range(num_instances):
        try:
            ...
            all_params["box_center"] = torch.cat(
                (
                    ((all_params["bbox_xyxy"][0] + all_params["bbox_xyxy"][2]) / 2.0)
                    .unsqueeze(0)
                    .unsqueeze(1),
                    ((all_params["bbox_xyxy"][1] + all_params["bbox_xyxy"][3]) / 2.0)
                    .unsqueeze(0)
                    .unsqueeze(1),
                ),
                dim=1,
            ...
            for ii in range(len(full_gt)):
                if full_gt[ii] == "shape":
                    ...
                    break
            all_params["pred_smpl_params"]["betas"] = all_params["pred_smpl_params"][
                "betas"
            ].unsqueeze(0)
            ...
            body_poses = (
                poses_tokenizer.decode_tokens(body_poses)
                .squeeze(2)
                .squeeze()
                .reshape(1, 23, 3, 3)
                .cpu()
            ...
            smpl_output = smpl(
                **{k: v.float().cpu() for k, v in smpl_params.items()}, pose2rot=False
            )
            ...
            for n in range(smpl_output.vertices.size(0)):
                ...
                scaled_focal_length = (
                    pose_model_cfg.EXTRA.FOCAL_LENGTH
                    / pose_model_cfg.MODEL.IMAGE_SIZE
                    * img_size.max()
                )
                pred_cam_t_full = (
                    cam_crop_to_full(
    ...
    try:
        input_img = (
            destandardize(
                mod_dict["rgb@224"]["tensor"].squeeze(),
    ...
    if len(all_verts) > 0:
        ...
        input_img_overlay = (
            0.5 * input_img[:, :, :3] * (1 - mask) + cam_view[:, :, :3] * mask
        )
...
def visualize_bboxes(img, bboxes_str, color=BOX_COLOR, thickness=2):
    ...
    for bbox in bboxes:
        ...
        ((text_width, text_height), _) = cv2.getTextSize(
            class_name.rstrip(), cv2.FONT_HERSHEY_SIMPLEX, 0.35, 1
        )
        cv2.rectangle(
            img,
...
def plot_text_in_square(ax, text, padding=0.5, fontsize=14, wrap_width=50):
    ...
    # Add the padding
    bbox_props = dict(
        boxstyle="square,pad=" + str(padding), facecolor="white", edgecolor="black"
    )
...
def text_to_pil_image(
    text, padding=0.5, fontsize=14, wrap_width=40, image_size=(512, 512)
):
    """
...
def plot_modality(dec_dict, key, ax, figscale=4.0):
    ...
    if "tok" in k or k == "rgb" or k == "human_poses" or k == "color_palette":
        ...
        plot_text_in_square(
            ax, modality, wrap_width=max(1, int(7 * figscale)), fontsize=11
        )
    elif k == "det":
...
def plot_conds_and_targets(
    ...
    for cond_idx, cond_domain in enumerate(cond_domains):
        ...
        axi.set_title(
            f"Conditioning: {MOD_PRINT_NAMES[cond_domain]}", fontsize=fs_titles
        )
    ...
    for sample_idx, dec_dict in enumerate(dec_dicts):
        for target_idx, target_domain in enumerate(target_domains):
            ...
            axi.set_title(
                f"{sample_idx+1}.{target_idx+1}: {MOD_PRINT_NAMES[target_domain]}",
                fontsize=fs_titles,
            )
    ...
    if save_path is not None:
        ...
        plt.savefig(
            save_path, bbox_inches="tight", dpi=dpi
...
def save_conds_and_targets(
    ...
    for variant_idx, dec_dict in enumerate(dec_dicts):
        ...
        for domain in cond_domains + target_domains:
            ...
            domain_save_dir = os.path.join(
                save_dir, "conds" if domain in cond_domains else "targets", domain
            )
            ...
            if "tok" in domain or domain in ["rgb", "human_poses", "color_palette"]:
                ...
                save_path = os.path.join(
                    domain_save_dir, f"{sample_idx:06d}{variant_suffix}.png"
                )
                img.save(save_path)
                ...
                if vis_det:
                    ...
                    bbox_img = visualize_bboxes(
                        np.ones((512, 512, 3)), dec_dict[domain], thickness=2
                    )
                    bbox_img = Image.fromarray(
                        (255 * bbox_img.clip(0, 1)).astype(np.uint8)
                    )
                    bbox_img.save(save_path)
                else:
                    # Save caption as text file
                    save_path = os.path.join(
                        domain_save_dir, f"{sample_idx:06d}{variant_suffix}.txt"
                    )
...
def plot_images_with_captions(
    images, captions, save_path=None, dpi=100, wrap_length=40, figscale=4.0
):
    """
    ...
    assert len(images) == len(
        captions
    ), "Number of images must match number of captions!"
    ...
    fig, axarr = plt.subplots(
        nrows=nrows, ncols=ncols, figsize=(figscale * ncols, figscale * nrows)
    )
