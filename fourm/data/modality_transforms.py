...
class NormalTransform(ImageTransform):
    ...
    def __init__(self, standardize_surface_normals=False):
        self.normal_mean = (
            (0.5, 0.5, 0.5)
            if not standardize_surface_normals
            else IMAGENET_SURFACE_NORMAL_MEAN
        )
        self.normal_std = (
            (0.5, 0.5, 0.5)
            if not standardize_surface_normals
            else IMAGENET_SURFACE_NORMAL_STD
        )
...
class SemsegTransform(ImageTransform):
    ...
    def semseg_to_tensor(self, img):
        ...
        if self.scale_factor != 1.0:
            target_height, target_width = int(img.height * self.scale_factor), int(
                img.width * self.scale_factor
            )
...
class SAMInstanceTransform(AbstractTransform):
    ...
    def remove_redundant_lines(self, orig_instance, instance):
        ...
        for p in instance:
            distance = cv2.pointPolygonTest(
                orig_instance, (p[0, 0].item(), p[0, 1].item()), measureDist=True
            )
            if distance >= 0:
    ...
    def crop_sample(self, sample, crop_coords):
        ...
        for instance in sample:
            ...
            instance_copy[:, :, 0] = np.clip(
                instance[:, :, 0], a_min=crop_region[0], a_max=crop_region[2]
            )
            instance_copy[:, :, 1] = np.clip(
                instance[:, :, 1], a_min=crop_region[1], a_max=crop_region[3]
            )
    ...
    def remove_tiny_instances(self, sample, image_size):
        ...
        for instance in sample:
            ...
            bbox_area_ratio = (
                (max_h - min_h) * (max_w - min_w) / (image_size[0] * image_size[1])
            )
    ...
    def get_binary_masks(self, sample):
        ...
        for i, instance in enumerate(sample):
            ...
            instance_copy[:, :, 0] = (
                (instance_copy[:, :, 0] - min_w) / (max_w - min_w) * self.mask_size
            )
            instance_copy[:, :, 1] = (
                (instance_copy[:, :, 1] - min_h) / (max_h - min_h) * self.mask_size
            )
    ...
    def preprocess(self, sample):
        if self.max_instance_n is None or len(sample) <= self.max_instance_n:
            ...
            indecies = np.random.choice(
                len(sample), size=self.max_instance_n, replace=False
            )
...
class DetectionTransform(AbstractTransform):
    ...
    def bboxes_hflip(self, bboxes: List[Tuple], image_size: Tuple, flip: bool):
        ...
        if flip:
            bboxes = [
                tuple(A.bbox_hflip(bbox[:4], rows=image_height, cols=image_width))
                + tuple(bbox[4:])
                for bbox in bboxes
            ]
    ...
    def bboxes_crop_and_resize(
        self, bboxes: List[Tuple], crop_coords: Tuple, orig_size: Tuple
    ):
        ...
        bboxes = A.core.bbox_utils.filter_bboxes(
            bboxes, rows=h, cols=w, min_visibility=self.min_visibility
        )
    ...
    def metadata_to_string(self, metadata, keys: List[str] = None):
        ...
        for key in keys:
            if key in self.image_dim_modalities:
                ...
                metadata_str = self.image_dim_to_string(
                    metadata, key, bin_size=self.image_dim_bin_size
                )
                ...
                metadata_str = self.float_metadata_to_string(
                    metadata, key, vmin, vmax, bins
                )
...
class HumanPoseTransform(AbstractTransform):
    ...
    def convert_humanpose_instance(self, instances, only_pose=False):
        ...
        if only_pose:  # used for tokenizer training for pose
            if len(instances) == 0:
                ...
                return torch.from_numpy(
                    np.array(instances["pred_smpl_params"]["body_pose"][0]).flatten()
                ).float()
        ...
        for ii in range(total_num_instances):
            instances_converted.append(
                ...
                + ["global"]
                + (
                    np.array(instances["pred_smpl_params"]["global_orient"][ii])
                    .flatten()
                    .tolist()
                )
                + ["pose"]
    ...
    def humanposes_crop_and_resize(
        ...
        for instance in humanposes:
            ...
            bbox_curr = np.array(bbox_curr)
            if np.all(bbox_curr[1::2] < 0) or np.all(
                bbox_curr[0::2] < 0
            ):  # bbox is out of range, remove it
                continue
            if np.all(bbox_curr[1::2] > 1.0) or np.all(
                bbox_curr[0::2] > 1.0
            ):  # bbox is out of range, remove it
                continue
...
class ColorPaletteTransform(AbstractTransform):
    ...
    def convert_palettes_to_string(self, all_palettes: List[Tuple]):
        ...
        colors.append(
            f"v1={round(len_palettes/3)}"
        )  # start with the length of the color palette to avoid confusion
...
class SAMInstanceTokTransform(AbstractTransform):
    ...
    def get_query_points(self):
        if self.point_order == "grid":
            ...
            raise ValueError(
                f"Query point order mode {self.point_order} is not supported."
            )
    ...
    def get_target_tokens(self, sample, query_points):
        ...
        for point in query_points:
            ...
            for i, (coords, tok, bbox) in enumerate(
                zip(instances_coords, tokens, bboxes)
            ):
...
class CropSettingsTransform(AbstractTransform):
    ...
    def preprocess(self, sample):
        raise NotImplementedError(
            "CropSettingsTransform does not support preprocessing"
        )
    ...
    def image_augment(
        ...
        raise NotImplementedError(
            "CropSettingsTransform is not meant to be used for image augmentation"
        )

    def postprocess(self, sample):
        raise NotImplementedError(
            "CropSettingsTransform does not support postprocessing"
        )
