...
def sample_cosine(min_val: float = 0, max_val: float = 1) -> float:
    ...
    return min_val + 0.5 * (max_val - min_val) * (
        1 + math.cos(math.pi * random.uniform(0, 1))
    )
...
class UnifiedMasking(object):
    def __init__(
        ...
        self.target_tokens_range = (
            to_2tuple(target_tokens_range) if target_tokens_range is not None else None
        )
        self.modality_info = modality_info
        self.num_modalities = len(modality_info)
        self.max_tries = max_tries
        self.min_tokens = torch.tensor(
            [mod["min_tokens"] for mod in modality_info.values()]
        )
        self.max_tokens = torch.tensor(
            [mod["max_tokens"] for mod in modality_info.values()]
        )
        self.mod_is_img = torch.tensor(
            [mod["type"] == "img" for mod in modality_info.values()]
        )

        # Dirichlet sampling (supports a mixture of multiple Dirichlet distributions)
        eps = 1e-9
        input_alphas = torch.tensor(
            [mod["input_alphas"] for mod in modality_info.values()]
        )
        input_alphas = rearrange(input_alphas, "nmod nmix -> nmix nmod")
        self.input_dirichlets = [
            Dirichlet(torch.clamp(input_alpha, min=eps)) for input_alpha in input_alphas
        ]
        target_alphas = torch.tensor(
            [mod["target_alphas"] for mod in modality_info.values()]
        )
    ...
    def input_token_budget(self, num_input_tokens, dir_idx=0):
        ...
        for i in range(self.max_tries):
            input_token_budget = (
                (self.input_dirichlets[dir_idx].sample() * num_input_tokens)
                .floor()
                .int()
            )
            ...
            input_token_budget += torch.bincount(
                self.input_dirichlets[dir_idx].sample_n(diff).argmax(dim=-1),
                minlength=len(input_token_budget),
            )
    ...
    def target_token_budget(self, input_token_budget, num_target_tokens, dir_idx=0):
        ...
        max_tokens_remaining = torch.where(
            self.mod_is_img,
            self.max_tokens - torch.tensor(input_token_budget),
            self.max_tokens,
        )
        max_tokens_remaining = torch.max(self.min_tokens, max_tokens_remaining)
        for i in range(self.max_tries):
            target_token_budget = (
                (self.target_dirichlets[dir_idx].sample() * num_target_tokens)
                .floor()
                .int()
            )
            ...
            target_token_budget += torch.bincount(
                self.target_dirichlets[dir_idx].sample_n(diff).argmax(dim=-1),
                minlength=len(target_token_budget),
            )

            # If token budget is over max tokens for a given modality, set it to max
            target_token_budget = torch.clamp(
                target_token_budget, max=max_tokens_remaining
            )
    ...
    def image_mask(
        ...
        first_mask_token = torch.argmin(
            target_mask
            + torch.arange(target_mask.shape[0], device=target_mask.device) * 1e-6
        )
        decoder_attention_mask[first_mask_token] = (
            ~target_mask
        ).sum()  # Equiv. to target budget
    ...
    def sequence_token_mask(
        ...
        seq_ids = (
            seq_ids + vocab_offset
        )  # Avoid overlap with sentinel tokens (needs to be substracted after decoding)

        # If input budget is 0, treat it as if the whole sequence is completely masked
        if input_budget == 0:
            keep_prob = 0.0
            input_seq_ids = []
            _, target_seq_ids = simple_span_masking(
                seq_ids, self.sentinel_to_id, keep_prob
            )
        else:
            if keep_scheme == "random":
                ...
                raise ValueError(
                    f"Invalid keep scheme for sequence masking: {keep_scheme}"
                )

            input_seq_ids, target_seq_ids = simple_span_masking(
                seq_ids, self.sentinel_to_id, keep_prob
            )
        ...
        if target_budget is None or len(target_seq_ids) <= target_budget:
            tensor[input_budget : input_budget + len(target_seq_ids)] = torch.tensor(
                target_seq_ids, dtype=torch.int
            )
            target_mask[input_budget : input_budget + len(target_seq_ids)] = 0
            decoder_attention_mask[
                input_budget : input_budget + len(target_seq_ids)
            ] = 1
        else:
            # Randomly choose sentinel token.
            sentinel_indices = [
                i
                for i, token_id in enumerate(target_seq_ids)
                if token_id in self.sentinel_ids
            ]
            ...
            if len(target_seq_ids) - sentinel_indices[chosen_sentinel] >= target_budget:
                target_seq_ids = target_seq_ids[
                    sentinel_indices[chosen_sentinel] : sentinel_indices[
                        chosen_sentinel
                    ]
                    + target_budget
                ]
    ...
    def sequence_mask(
        ...
        if isinstance(sequence, str):
            ...
            cumulative_token_count = np.cumsum(
                np.array([len(chunk) for chunk in seq_ids])
            )
            seq_ids = [
                chunk
                for (chunk, token_count) in zip(seq_ids, cumulative_token_count)
        ...
        if input_budget == 0:
            ...
            input_seq_ids, target_seq_ids = span_masking_fn(
                seq_ids, self.sentinel_to_id, keep_prob
            )
            # Keep lowering the keep_prob while we are over-budget
            while len(input_seq_ids) > input_budget:
    ...
    def sequence_emb_mask_span(
        ...
        if input_budget == 0:
            keep_prob = 0.0
            fake_input_seq_ids = []
            _, fake_target_seq_ids = simple_span_masking(
        ...
        for i_, fake_id in enumerate(fake_input_seq_ids):
            if fake_id in self.sentinel_ids:
                tensor[i_, :] = torch.zeros_like(
                    emb_tensor[0, :]
                )  # TODO replace to learned embeddings later
            else:
    ...
    def __call__(self, mod_dict):
        ...
        for (mod_name, mod_info), input_budget, target_budget in zip(
            ...
            mod_name_load = (
                mod_name if mod_name in mod_dict else get_modality_prefix(mod_name)
            )
            if mod_type == "img":
                ...
                keep_scheme = (
                    "random" if ("keep" not in mod_info) else mod_info["keep"][dir_idx]
                )
...
class TransferMasking(object):
    ...
    def input_sequence(self, sequence_str: str, max_tokens: int):
        ...
        tensor[max_tokens : max_tokens + len(target_seq_ids)] = torch.tensor(
            target_seq_ids, dtype=torch.int
        )
    ...
    def __call__(self, mod_dict):
        ...
        for mod_name, mod_info in self.modality_info.items():
            ...
            if mod_type == "img" and mod_name in self.input_modalities:
                masked_mod_dict[mod_name] = self.input_image(
                    mod_dict[mod_name], mod_info["max_tokens"]
                )
            elif mod_type == "img" and mod_name in self.target_modalities:
                masked_mod_dict[mod_name] = self.target_image(
                    mod_dict[mod_name], mod_info["max_tokens"]
                )
            elif mod_type == "seq" and mod_name in self.input_modalities:
                masked_mod_dict[mod_name] = self.input_sequence(
                    mod_dict[mod_name], mod_info["max_tokens"]
                )
