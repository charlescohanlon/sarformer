...
def build_fm_pretraining_dataset(
    ...
    modalities_without_vq = [
        mod
        for mod in all_domains
        if not modality_info[mod].get("requires_tokenizer", False)
    ]
    # If we are using a pre-tokenized modality, we default to pre-computed crop settings
    if any(
        [modality_info[domain].get("pretokenized", False) for domain in all_domains]
    ):
...
def wds_decoder(key, value):
    if key == "png" or key.endswith(".png"):
        ...
        img = Image.open(io.BytesIO(value))
        return img
    elif key == "tif" or key.endswith(".tif"):
        img = Image.open(io.BytesIO(value))
        return img
...
def filter_metadata(
    sample, metadata=["__key__", "__url__", "file_name", "class_name", "class_idx"]
):
...
def rename_modalities(sample, modality_paths):
    ...
    """
    return {
        out_path: sample[loaded_path]
        for out_path, loaded_path in modality_paths.items()
    }
...
def build_huggingface_pretraining_dataloader(
    ...
    dataset = split_dataset_by_node(
        dataset, rank=get_rank(), world_size=get_world_size()
    )
    dataset = dataset.shuffle(seed=shuffle_seed, buffer_size=shuffle_buffer_load)

    modality_info = {
        mod: modality_info[mod] for mod in modality_info if mod in all_domains
    }
    ...
    datapipe = wds.DataPipeline(
        dataset,
        (
            map(text_to_caption) if rename_text_to_caption else map(identity)
        ),  # Rename "text" to "caption"
        map(filter_metadata),  # Remove non-task keys
        map(transform),  # Apply data augmentation and masking
        (
...
def make_empty_mod_dict(modality_info):
    ...
    for mod_name, mod_info in modality_info.items():
        ...
        if "num_channels" in mod_info and "input_size" in mod_info:
            ...
            empty_mod["tensor"] = torch.zeros(
                (max_tokens, orig_emb_dim), dtype=torch.float32
            )
        ...
        empty_mod["decoder_attention_mask"] = torch.zeros(
...
class MixtureDataset(IterableDataset):
    def __init__(self, data_iters, weights, modality_info):
        ...
        self.data_iters = [
            iter(data_iter) for data_iter in data_iters
    ...
    def __iter__(self):
        while True:
            dataset_idx = np.random.choice(
                len(self.sampling_probs), p=self.sampling_probs
            )
            try:
...
def build_mixture_dataloader(
    ...
    mixture_loader = wds.WebLoader(
        mixture_pipe, num_workers=num_workers, batch_size=None
    )
