...
def get_image_tokenizer(
    ...
    """
    if return_None_on_fail and not os.path.exists(
        os.path.join(tokenizers_root, f"{tokenizer_id}.pth")
    ):
        return None
    ...
    if (
        "CLIP" in ckpt["args"].domain
        or "DINO" in ckpt["args"].domain
        or "ImageBind" in ckpt["args"].domain
    ):
    ...
    ckpt["args"].image_size = getattr(ckpt["args"], "input_size", None) or getattr(
        ckpt["args"], "input_size_max", None
    )
    ...
    if "cls_emb.weight" in ckpt["model"]:
        ckpt["args"].n_labels, ckpt["args"].n_channels = n_labels, n_channels = ckpt[
            "model"
        ]["cls_emb.weight"].shape
    ...
    if encoder_only:
        ...
        ckpt["model"] = {
            ...
            if "decoder" not in k and "post_quant_proj" not in k
        }
    else:
