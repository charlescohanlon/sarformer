...
def get_num_layer_for_vit(var_name, num_max_layer):
    if var_name in ("cls_token", "mask_token", "pos_embed", "global_tokens"):
        ...
        return 0
    elif var_name.startswith("input_adapters") or var_name.startswith(
        "encoder_embeddings"
    ):
        return 0
    elif var_name.startswith("rel_pos_bias"):
        return num_max_layer - 1
    elif var_name.startswith("blocks") or (
        var_name.startswith("encoder.") and not var_name.startswith("encoder_norm")
    ):
...
def get_num_layer_for_fm(
    var_name, num_enc_layers, num_dec_layers, last_layer_mod_emb=False
):
    ...
    if var_name.startswith("encoder_embeddings"):
        ...
        not last_layer_mod_emb
        and var_name.startswith("decoder_embeddings.")
        and "mod_emb" in var_name
    ):
        return num_enc_layers
...
def create_optimizer(
    ...
    if isinstance(model, torch.nn.Module):
        ...
        print(
            "WARNING: Weight decay assignment is skipped. All layers are assigned a weight decay value."
        )
        parameters = [
            {
                "params": [
