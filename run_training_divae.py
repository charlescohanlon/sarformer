...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no_resolution_cond", action="store_false", dest="resolution_cond"
    )
    ...
    for step, x in enumerate(
        ...
        with torch.amp.autocast(
            ...
            if prediction_type == "sample":
                ...
                target = unwrap_model(model).noise_scheduler.get_velocity(
                    clean_images, noise, timesteps
                )
            elif prediction_type == "v_prediction-epsilon_loss":
                target = noise
                model_output = unwrap_model(model).noise_scheduler.get_noise(
                    noisy_images, model_output, timesteps
                )
