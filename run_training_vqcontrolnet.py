...
def main(args):
    ...
    if args.use_wds:
        ...
        transforms_train = UnifiedDataTransform(
            transforms_dict=MODALITY_TRANSFORMS_VQCONTROLNET,
            image_augmenter=image_augmenter_train,
        )
...
def train_one_epoch(
    ...
    for step, x in enumerate(
        ...
        clean_input = unwrap_model(model).vae_encode(
            F.interpolate(
                clean_images, args.input_size_sd, mode="bilinear", align_corners=False
            )
        )
        ...
        noisy_input = unwrap_model(model).noise_scheduler.add_noise(
...
def eval_metrics(
    ...
    for x in metric_logger.log_every(
        ...
        if domain in ["rgb", "normal"]:
            gt = destandardize(clean_images, mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))
            output = unwrap_model(model).vae_decode(output)
            output = F.interpolate(
                output, gt.shape[-1], mode="bilinear", align_corners=False
            )
...
def eval_image_log(
    ...
    if utils.is_main_process():
        ...
        for x in data_loader:
            ...
            if domain in ["rgb", "normal"]:
                gt = destandardize(
                    clean_images, mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)
                )
                output = unwrap_model(model).vae_decode(output)
                output = F.interpolate(
                    output, gt.shape[-1], mode="bilinear", align_corners=False
            ...
            if domain in ["rgb", "normal"]:
                gt_bytes = (
                    255 * gt.permute(0, 2, 3, 1).clamp(0, 1).cpu().numpy()
                ).astype(np.uint8)
                reconst_bytes = (
                    255 * reconst.permute(0, 2, 3, 1).clamp(0, 1).cpu().numpy()
                ).astype(np.uint8)
