...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="Number of repeats for webdataset loader to improve efficiency",
    )
    parser.add_argument(
        "--wds_shuffle_buffer_tar",
        default=1_000,
        type=int,
        help="Webdatasets shuffle buffer after loading tar files",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="Needed for DDP when using webdatasets",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Disable Inception metric during eval",
    )
    parser.add_argument(
...
def get_perceptual_loss(
    args: argparse.Namespace, device: Union[torch.device, str]
) -> Optional[nn.Module]:
    ...
    if args.percept_loss_type == "lpips":
        ...
        percept_loss.percept_transform = (
            lambda x: x
        )  # No transform needed for LPIPS. Expects [-1, 1] range
    else:
...
def main(args: argparse.Namespace) -> None:
    ...
    if "human_poses" not in args.domain:
        image_augmenter_train = RandomCropImageAugmenter(
            ...
            main_domain=args.all_domains[0],
    ...
    if args.use_wds:
        ...
        num_training_steps_per_epoch = args.dataset_size // (
            args.batch_size * num_tasks
        )
    else:
    ...
    if args.eval_data_path:
        if "human_poses" not in args.domain:
            image_augmenter_val = CenterCropImageAugmenter(
                target_size=args.input_size, main_domain=args.all_domains[0]
            )
        else:
    ...
    train_res_choices = list(
        range(
            args.input_size_min,
            args.input_size_max + args.resolution_step,
    ...
    eval_image_sizes = set(
        [*args.input_size_eval, train_res_choices[0], train_res_choices[-1]]
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.step_eval or (epoch + 1 == args.epochs):
            ...
            launch_eval_metrics = (
                ...
                and (
                    (epoch % args.eval_metrics_freq == 0) or (epoch + 1 == args.epochs)
                )
            )
            launch_eval_image_log = (
                ...
                and (
                    (epoch % args.eval_image_log_freq == 0)
...
def prepare_inputs(
    ...
    if image_size is not None and domain not in ["human_poses", "sam_mask"]:
        if "semseg" in domain:
            images = (
                F.interpolate(images.unsqueeze(1).float(), image_size, mode="nearest")
                .to(images.dtype)
                .squeeze(1)
            )
        else:
            images = F.interpolate(
                images, image_size, mode="bilinear", align_corners=False
            )
...
def train_one_epoch(
    ...
    for step, x in enumerate(
        ...
        with torch.amp.autocast(
            ...
            if percept_loss_fn is not None and percept_loss_weight > 0:
                ...
                percept_loss = percept_loss_fn(
                    transform(model_output).to(dtype_percept),
                    transform(images).to(dtype_percept),
                ).mean()
            else:
        ...
        loss = (
            ...
            + percept_loss_weight * percept_loss
        )
...
def evaluate(
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    for step, x in enumerate(
        metric_logger.log_every(data_loader, print_freq=10, header=f"{prefix} ")
    ):
...
def eval_metrics(
    ...
    for x in metric_logger.log_every(
        data_loader, print_freq=10, header=f"{prefix} Image metrics @{eval_size}:"
    ):
        ...
        if domain in ["rgb", "normal"]:
            ...
            gt = destandardize(images[:, :3], mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))
            ...
            gt = destandardize(images[:, :1], mean=(0.5,), std=(0.5,))
            ...
            gt = destandardize(images[:, :2], mean=(0.5, 0.5), std=(0.5, 0.5))
        ...
        if utils.is_main_process() and log_writer is not None:
            ...
            if hasattr(unwrap_model(model).quantize, "_codebook") and hasattr(
                ...
                log_writer.wandb_safe_log(
                    {prefix + "CodebookEMAClusterSizes" + suffix: wandb.Image(fig)},
                    commit=False,
                )
                plt.close(fig)
            ...
            log_writer.wandb_safe_log(
                {prefix + "TokenUsageFreq" + suffix: wandb.Image(fig)}, commit=False
            )
            plt.close(fig)
    ...
    if utils.is_main_process():
        ...
        for x in data_loader:
            ...
            if domain in ["rgb", "normal"]:
                # 3-channel domains in [-1,1]
                gt = destandardize(
                    images[:, :3], mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)
                )
                reconst = destandardize(
                    output[:, :3], mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)
                )
                ...
                gt[:, [0, 1]] = destandardize(
                    images[:, :2], mean=(0.5, 0.5), std=(0.5, 0.5)
                )
                reconst = torch.zeros(
                    B, 3, H, W, device=images.device, dtype=images.dtype
                )
                ...
                gt_onehot = rearrange(
                    F.one_hot(images, num_classes=num_classes), "b h w c -> b c h w"
                ).float()
                ...
                gt = np.stack(
                    [pca_visualize(feat) for feat in images]
                )  # [B, N_H, N_W, 3]
                reconst = np.stack(
                    [pca_visualize(feat) for feat in output]
                )  # [B, N_H, N_W, 3]
                gt = F.interpolate(
                    torch.tensor(gt).permute(0, 3, 1, 2), eval_size, mode="nearest"
                )
                reconst = F.interpolate(
                    torch.tensor(reconst).permute(0, 3, 1, 2), eval_size, mode="nearest"
                )
            elif "sam_mask" in domain:
                gt = images[:, :1]
                reconst = torch.sigmoid(output)[:, :1]
        ...
        if len(gt_imgs) > 0:
            log_writer.wandb_safe_log(
                {
                    f"{prefix} GT @{eval_size}": wandb.Image(
                        make_grid(gt_imgs[:num_logged_images])
                    )
                },
                commit=False,
            )
            log_writer.wandb_safe_log(
                {
                    f"{prefix} Reconst @{eval_size}": wandb.Image(
