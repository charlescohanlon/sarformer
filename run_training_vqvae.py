...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1.0,
        type=float,
        help="The weight of the orthogonal regularization in the code loss, (default: %(default)s)",
    )
    parser.add_argument(
        "--orthogonal_reg_active_codes_only",
        action="store_true",
        help="Only apply orthogonal regularization to active codes, (default: %(default)s)",
    )
    parser.add_argument(
        "--orthogonal_reg_max_codes",
        default=None,
        type=Optional[int],
        help="The maximum number of codes to apply orthogonal regularization to (None for all, default: %(default)s)",
    )
    ...
    parser.add_argument(
        "--hflip", default=0.5, type=float, help="Prob of horizontal flip"
    )
    parser.add_argument(
        "--vflip", default=0.5, type=float, help="Prob of vertical flip"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Evaluate on a step basis",
    )
    parser.add_argument(
        "--epoch_eval",
        action="store_false",
        dest="step_eval",
        help="Evaluate on an epoch basis",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="frequency of evaluation (in iterations or epochs)",
    )
    parser.add_argument(
        "--eval_metrics_freq",
        default=1,
        type=int,
        help="frequency of evaluation metrics (in iterations or epochs)",
    )
    parser.add_argument(
        "--eval_image_log_freq",
        default=5,
    ...
    parser.add_argument(
        "--log_codebook_usage", action="store_true", help="Log the codebook usage"
    )
    parser.add_argument(
        "--no_codebook_usage",
        action="store_false",
    ...
    parser.add_argument("--log_fid", action="store_true", help="Log FID")
    parser.add_argument("--no_log_fid", action="store_false", dest="log_fid")
    parser.set_defaults(log_fid=False)

    parser.add_argument(
        "--log_fid_last_epoch", action="store_true", help="Log FID on the last epoch"
    )
    parser.add_argument(
        "--no_log_fid_last_epoch", action="store_false", dest="log_fid_last_epoch"
    )
    parser.set_defaults(log_fid_last_epoch=False)
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Extra wandb tags, separated by a double hyphen",
    )
...
def get_model(args: argparse.Namespace, device: Union[torch.device, str]) -> VQVAE:
    ...
    tokens_per_image = (args.input_size // args.patch_size) ** 2
    codebook_size_int = (
        np.prod([int(d) for d in args.codebook_size.split("-")])
        if isinstance(args.codebook_size, str)
        else args.codebook_size
    )
    uniform_token_count_per_batch = (
        total_batch_size * tokens_per_image / codebook_size_int
    )
    ...
    model = VQVAE(
        ...
        sync_codebook=args.distributed,  # Training on multiple GPUs
...
def main(args: argparse.Namespace) -> None:
    ...
    # Done aftrer modality info set up
    ...
    image_augmenter_train = CropImageAugmenter(
        target_size=args.input_size, hflip=args.hflip, vflip=args.vflip
    )
    ...
    if args.eval_data_path:
        image_augmenter_val = CropImageAugmenter(
            target_size=args.input_size, hflip=args.hflip, vflip=args.vflip
        )
        ...
        data_loader_val, data_loader_metrics, data_loader_image_log = None, None, None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.step_eval or is_last_epoch:
            launch_eval = (data_loader_val is not None) and (
                ((epoch + 1) % args.eval_freq == 0) or is_last_epoch
            )
            launch_eval_metrics = (
                (data_loader_metrics is not None)
                and args.eval_metrics_freq > 0
                and (((epoch + 1) % args.eval_metrics_freq == 0) or is_last_epoch)
            )
            launch_eval_image_log = (
                (data_loader_image_log is not None)
                and args.eval_image_log_freq > 0
                and (((epoch + 1) % args.eval_image_log_freq == 0) or is_last_epoch)
            )
            eval_stats = launch_evals(
                ...
                log_writer=log_writer,
                log_codebook_usage=args.log_codebook_usage,
                log_fid=args.log_fid or (args.log_fid_last_epoch and is_last_epoch),
            )
...
def compute_reconst_loss(
    model_output: torch.Tensor, images: torch.Tensor, loss_fn: str
) -> torch.Tensor:
...
def train_one_epoch(
    ...
    """Perform one training epoch and return stats. At the specified intervals,
    ...
    Args:
        ...
        log_fid: [Eval] Whether to compute and log FID.
    ...
    for step, x in enumerate(
        metric_logger.log_every(
            data_loader, print_freq, iter_len=loader_len, header=header
        )
    ):
        ...
        if step_eval:
            launch_evaluate = (
                (data_loader_val is not None) and (it % eval_freq == 0) and (it != 0)
            )
            launch_eval_metrics = (
                (data_loader_metrics is not None)
        ...
        if lr_schedule_values is not None or wd_schedule_values is not None:
            for param_group in optimizer.param_groups:
                if lr_schedule_values is not None and len(lr_schedule_values) > 0:
                    param_group["lr"] = lr_schedule_values[it] * param_group["lr_scale"]
                if (
                    wd_schedule_values is not None
                    and param_group["weight_decay"] > 0
                    and len(wd_schedule_values) > 0
                ):
                    param_group["weight_decay"] = wd_schedule_values[it]

        images = x[domain].to(device)
        images = mask_out_samples(
            images, x.get("mask_valid", None), mask_value=mask_value
        )
...
def launch_evals(
    ...
    Args:
        ...
        log_fid: Whether to compute and log FID.
    ...
    if launch_eval:
        ...
        eval_stats = eval_wrap(model, "[Eval]")
        all_eval_stats.update(eval_stats)

        if model_ema is not None:
            eval_stats = eval_wrap(unwrap_model(model_ema), "[EMA Eval]")
            all_eval_stats.update(eval_stats)
    ...
    if launch_eval_metrics:
        ...
        def eval_metrics_wrap(model, prefix):
            return eval_metrics(
                model=model,
                ...
                dtype=dtype,
                mask_value=mask_value,
                compute_on_cpu=device.type == "cpu",
                log_writer=log_writer,
                prefix=prefix,
                log_codebook_usage=log_codebook_usage,
                log_fid=log_fid,
        ...
        eval_metrics_results = eval_metrics_wrap(model, "[Eval]")
        all_eval_stats.update(eval_metrics_results)

        if model_ema is not None:
            eval_metrics_results = eval_metrics_wrap(
                unwrap_model(model_ema), "[EMA Eval]"
            )
            all_eval_stats.update(eval_metrics_results)
    ...
    if launch_eval_image_log:
        ...
        def eval_image_log_wrap(model, prefix):
            eval_image_log(
                model=model,
        ...
        eval_image_log_wrap(model, "[Eval]")

        if model_ema is not None:
            eval_image_log_wrap(unwrap_model(model_ema), "[EMA Eval]")
...
def eval(
    ...
    """Perform one evaluation epoch and return stats.
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    for x in metric_logger.log_every(data_loader, print_freq=10, header=f"{prefix} "):
        with torch.amp.autocast(
            device_type, dtype=dtype, enabled=dtype != torch.float32
        ):
            images = x[domain].to(device)
            images = mask_out_samples(
                images, mask_valid=x.get("mask_valid", None), mask_value=mask_value
            )
            model_output, code_loss = model(images)
...
def eval_metrics(
    ...
    if "rgb" == domain:
        ...
        fid_metric, lpips_metric = None, None
    ...
    for x in metric_logger.log_every(
        data_loader, print_freq=10, header=f"{prefix} Image metrics:"
    ):
        images = x[domain].to(device)
        images = mask_out_samples(
            images, mask_valid=x.get("mask_valid", None), mask_value=mask_value
        ...
        with torch.no_grad(), torch.amp.autocast(
            device.type, dtype, enabled=dtype != torch.float32
        ):
        ...
        if domain == "rgb":
            ...
            gt = destandardize(images[:, :3], mean=NAIP_MEAN, std=NAIP_STD)
        ...
        # all supported modalities are normalized from the start
        gt = gt.clamp(0.0, 1.0)
    ...
    results[prefix + "MSE"] = mse_metric.compute().item()
    results[prefix + "MAE"] = mae_metric.compute().item()
    results[prefix + "PSNR"] = psnr_metric.compute().item()
    if ms_ssim_metric is not None:
        results[prefix + "MS-SSIM"] = ms_ssim_metric.compute().item()
    if fid_metric is not None:
        results[prefix + "FID"] = fid_metric.compute().item()
    if lpips_metric is not None:
        results[prefix + "LPIPS"] = lpips_metric.compute().item()
    ...
    if log_codebook_usage:
        ...
        if utils.is_dist_avail_and_initialized():
            global_tokens = [
                torch.zeros_like(local_tokens) for _ in range(utils.get_world_size())
            ]
            dist.all_gather_object(global_tokens, local_tokens)
            global_tokens = torch.cat(global_tokens, dim=0)
        else:
            global_tokens = local_tokens
        codebook_usage = compute_codebook_usage(  # TODO: figure out why this works (window size seems to big)
            global_tokens,
            codebook_size=unwrap_model(model).quantize.codebook_size,
            # Compute codebook usage over 256 images (to keep it consistent across different batch sizes)
            window_size=256 * tokens.shape[1:].numel(),
        )
        results[prefix + "CodebookUsage"] = codebook_usage
        ...
        if utils.is_main_process() and log_writer is not None:
            ...
            if hasattr(unwrap_model(model).quantize, "_codebook") and hasattr(
                ...
                plt.hist(
                    unwrap_model(model).quantize._codebook.cluster_size.cpu().numpy(),
                    bins=100,
                )
                ...
                log_writer.wandb_safe_log(
                    {prefix + "CodebookEMAClusterSizes": wandb.Image(fig)},
                    commit=False,
                )
                plt.close(fig)
            ...
            log_writer.wandb_safe_log(
                {prefix + "TokenUsageFreq": wandb.Image(fig)}, commit=False
            )
            plt.close(fig)
...
def eval_image_log(
    ...
    if log_writer is None or num_logged_images == 0:
    ...
    if utils.is_main_process():
        ...
        for x in data_loader:
            ...
            if domain == "rgb":
                ...
                # Depth is already minmax scaled
                reconst = output[:, :1].clamp(0, 1)
            else:
                raise NotImplementedError(
                    f"Domain {domain} not supported for plotting."
                )
            ...
            if gt.ndim == 4 and gt.shape[1] == 3:
                ...
                gt_bytes = (255 * gt.float()[:, 0].clamp(0, 1).cpu().numpy()).astype(
                    np.uint8
                )
                reconst_bytes = (
                    255 * reconst.float()[:, 0].clamp(0, 1).cpu().numpy()
                ).astype(np.uint8)
        ...
        if len(gt_imgs) > 0:
            log_writer.wandb_safe_log(
                {f"{prefix} GT": wandb.Image(make_grid(gt_imgs[:num_logged_images]))},
                commit=False,
            )
            log_writer.wandb_safe_log(
                {
                    f"{prefix} Reconst": wandb.Image(
                        make_grid(reconst_imgs[:num_logged_images])
                    )
                },
                commit=False,
            )
