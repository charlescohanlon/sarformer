...
from fourm.utils.plotting_utils import (
    decode_dict,
...
def get_args(args=None):
    config_parser = parser = argparse.ArgumentParser(
        description="Generation Config", add_help=False
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    parser.add_argument(
        "--no_activate_controlnet", action="store_false", dest="activate_controlnet"
    )
    ...
    parser.add_argument(
        "--image_size", default=224, type=int, help="Image size. (default: %(default)s)"
    )
    parser.add_argument(
        "--patch_size",
        default=16,
    ...
    parser.add_argument(
        "--data_path",
        default="/mnt/datasets/cc12_multitask_224/val",
...
def load_tokenizers(args, device):
    ...
    if args.imagebind_glob_tok_id:
        toks["tok_imagebind_global"] = load_model(
            args.imagebind_glob_tok_id, VQVAE, device
        )
...
def get_dataset(args, text_tokenizer):
    ...
    # For caption->X generation using Parti Prompts
        ...
        dataset = PartiPromptsDataset(
            text_tokenizer,
            max_length=128,
            parti_prompts_t5_embs=args.parti_prompts_t5_embs,
    ...
    # Otherwise, construct CC12M/IN1K-like pre-tokenized dataset
        ...
        if "tok" not in "-".join(args.loaded_domains):
            ...
            image_augmenter = PreTokenizedImageAugmenter(
                target_size=args.image_size, no_aug=True
            )
        ...
        transform = transforms.Compose(
            [
                UnifiedDataTransform(
                    transforms_dict=modality_transforms, image_augmenter=image_augmenter
                ),
                UnifiedMasking(
    ...
    dataset = SubsampleDatasetWrapper(
        dataset, dataset_size=args.num_samples, seed=0, return_orig_idx=True
    )

    return dataset


def create_superres_input(
    out_dict,
    sr_cond_domains,
    sr_target_domains,
    sr_tokens_per_target,
    ...
    for target_mod, ntoks in zip(sr_target_domains, sr_tokens_per_target):
        superres_sample = init_empty_target_modality(
            superres_sample, MODALITY_INFO, target_mod, 1, ntoks, device
        )
...
def main(args):
    ...
    args.output_dir = os.path.join(
        args.output_dir,
        args.data_name,
        f"{args.name}--{args.sr_name}" if args.sr_name else args.name,
    )
    ...
    # Generation parameters
    args.cond_domains = sorted(
        list(string_to_list(args.cond_domains, dtype=str, delim=delim))
    )
    ...
    args.decoding_steps = repeat_if_necessary(
        string_to_list(args.decoding_steps, dtype=int, delim=delim), n_targets
    )
    ...
    args.temps = repeat_if_necessary(
        string_to_list(args.temps, dtype=float, delim=delim), n_targets
    )
    args.temp_schedules = repeat_if_necessary(
        string_to_list(args.temp_schedules, dtype=str, delim=delim), n_targets
    )
    args.cfg_scales = repeat_if_necessary(
        string_to_list(args.cfg_scales, dtype=float, delim=delim), n_targets
    )
    args.cfg_schedules = repeat_if_necessary(
        string_to_list(args.cfg_schedules, dtype=str, delim=delim), n_targets
    )
    ...
    if args.sr_cond_domains is None:
        ...
        args.sr_cond_domains = sorted(
            list(string_to_list(args.sr_cond_domains, dtype=str, delim=delim))
        )
    args.sr_target_domains = string_to_list(
        args.sr_target_domains, dtype=str, delim=delim
    )
    args.sr_all_domains = sorted(
        list(set(args.sr_cond_domains) | set(args.sr_target_domains))
    )
    ...
    args.sr_autoregression_schemes = repeat_if_necessary(
        string_to_list(args.sr_autoregression_schemes, dtype=str, delim=delim),
        sr_n_targets,
    )
    ...
    args.sr_token_decoding_schedules = repeat_if_necessary(
        string_to_list(args.sr_token_decoding_schedules, dtype=str, delim=delim),
        sr_n_targets,
    )
    args.sr_temps = repeat_if_necessary(
        string_to_list(args.sr_temps, dtype=float, delim=delim), sr_n_targets
    )
    args.sr_temp_schedules = repeat_if_necessary(
        string_to_list(args.sr_temp_schedules, dtype=str, delim=delim), sr_n_targets
    )
    args.sr_cfg_scales = repeat_if_necessary(
        string_to_list(args.sr_cfg_scales, dtype=float, delim=delim), sr_n_targets
    )
    ...
    if global_rank == 0 and args.log_wandb:
        ...
        args.wandb_run_name = (
            f"{args.name}--{args.sr_name}--{args.data_name}--{args.wandb_run_name}"
        )
    ...
    if args.output_dir and utils.is_main_process():
        with open(
            os.path.join(args.output_dir, "log_eval.txt"), mode="a", encoding="utf-8"
        ) as f:
...
def generate(
    ...
    if "tok_rgb" in args.target_domains:
        ...
        if "caption" in args.cond_domains:
            clip_metric = CLIPScore(
                model_name_or_path="openai/clip-vit-large-patch14", sync_on_compute=True
            ).to(device)
    ...
    for sample, sample_idx in metric_logger.log_every(
        data_loader, print_freq=1, header="Generation:"
    ):
        ...
        if fid_metric is not None or fid_metric_sr is not None:
            rgb_real = (255 * destandardize(sample["rgb"]["tensor"])).to(torch.uint8)
        ...
        for target_mod, ntoks in zip(args.target_domains, args.tokens_per_target):
            sample = init_empty_target_modality(
                sample, MODALITY_INFO, target_mod, 1, ntoks, device
            )
        ...
        for i in range(args.num_variations):
            ...
            if inception_metric is not None:
                rgb_pred = (
                    TF.to_tensor(255 * dec_dict["tok_rgb"])
                ...
                if clip_metric is not None:
                    caption_trunc = truncate_caption_for_clip(
                        dec_dict["caption"][0], clip_metric.processor.tokenizer
                    )
            ...
            if gen_sampler_sr is not None:
                ...
                if inception_metric_sr is not None:
                    rgb_pred = (
                        TF.to_tensor(255 * dec_dict_sr["tok_rgb@448"])
                        .to(dtype=torch.uint8, device=device)
                        .unsqueeze(0)
                    )
                    rgb_pred = TF.resize(rgb_pred, size=args.image_size_metrics)
        ...
        if args.num_log_images == "all" or (
            utils.is_main_process() and logged_images_count < int(args.num_log_images)
        ):
            plot_conds_and_targets(
                args.cond_domains,
                args.target_domains,
                ...
                save_path=os.path.join(
                    args.output_dir, "plots", f"{sample_idx:06d}.jpg"
                ),
            )
            for sr_idx, sr_dec_dict in enumerate(dec_dicts_sr):
                plot_conds_and_targets(
                    ...
                    [sr_dec_dict],
                    save_path=os.path.join(
                        args.output_dir, "plots", f"{sample_idx:06d}_sr{sr_idx}.jpg"
                    ),
                )
